      ...
      <div className="pt-15">
        ...
        <div className="mx-auto pt-10 max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
          <div className="rounded-lg md:w-2/3">
            {items.length > 0 ? (
              items.map((item, id) => <CartItem key={id} item={item} />)
            ) : (
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-10 shadow-md md:mt-0 md:w-1/3">
            ...
            <button
              className="mt-6 w-full rounded-md bg-blue-500 py-1.5 font-medium text-blue-50 hover:bg-blue-600 disabled:bg-gray-700 disabled:cursor-not-allowed"
              disabled={cartTotal === 0 || mutation.isPending ? true : false}
              onClick={() => {
                handleSubmit();
              }}
            >
